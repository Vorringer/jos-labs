...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
	...
	for(; ph < eph; ph++)
	{
		if(ph->p_type == ELF_PROG_LOAD)
		{
			...
			{
				*(char*)(ph->p_va + i) = *(char*)(binary + ph->p_offset + i);
			}
			...
			//heap initialization
			e->brk = ROUNDUP(ph->p_va + ph->p_memsz, PGSIZE);
		}
	}
	...
	if(p == NULL) panic("no enougf memory for user stack");
...
env_create(uint8_t *binary, size_t size, enum EnvType type)
{
	// LAB 3: Your code here.
	struct Env *e = NULL;
	if(env_alloc(&e, 0) < 0) panic("cannot create a new env");
	e->env_type = type;
